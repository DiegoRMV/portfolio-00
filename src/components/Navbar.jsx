	...
	return (
		<nav className="max-w-[1440px] w-full z-50 fixed top-0 right-0 left-0 mx-auto dark:bg-black dark:text-white flex justify-center bg-white">
			<div className="flex justify-between items-center align-middle w-full max-w-screen-xl h-16">
				<div className="md:hidden z-10 mx-4">
				...
				<div
					onClick={() => setNav(!nav)}
					className="cursor-pointer p-4 z-10 md:hidden text-3xl transition-all"
				>
					{nav ? <FaTimes /> : <FaBars />}
				</div>
				{nav ? (
					<ul className="md:hidden flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen dark:bg-gray-950 bg-white transition-all">
						{links.map(({ id, link }) => (
							<li
								key={id}
								className="px-4 cursor-pointer py-6 text-3xl font-fireCode hover:text-green1"
							>
								<Link
									onClick={() => setNav(!nav)}
									to={link}
